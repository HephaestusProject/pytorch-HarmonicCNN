...
class MTATDataset(data.Dataset):
	...
	def get_audio_tag(self, index):
		...
		downsample_resample = torchaudio.transforms.Resample(sr, self.sampling_rate, resampling_method='sinc_interpolation')
		audio_tensor = downsample_resample(waveform)
		random_idx = int(np.floor(np.random.random(1) * (audio_tensor.shape[1]-self.input_length)))
		audio_tensor = audio_tensor[:,random_idx:random_idx+self.input_length].squeeze(0)
...
def get_audio_loader(root, batch_size, input_length, split='TRAIN', num_workers=0):
	data_loader = data.DataLoader(dataset=MTATDataset(root, split=split, input_length=input_length),
